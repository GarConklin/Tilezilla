...
.EXAMPLE
  .\tools\scripts\run-retry-loop-audit-solves.ps1
  .\tools\scripts\run-retry-loop-audit-solves.ps1 -ContinueOnError
#>
param(
  [string]$RepoRoot = "",
...
if ($failed.Count -eq 0) {
  ...
  $syncArgs = @("compose", "-f", "docker-compose.yml", "run", "--rm", "web", "node", "tools/scripts/sync-catalog-path-count-from-solves.js", "--apply")
  $prevEap = $ErrorActionPreference
  $ErrorActionPreference = "Continue"
