...
.DESCRIPTION
  ...
  4. After all finish:
       .\tools\scripts\run-6x6-enumerate.ps1 -SyncCatalogOnly -NoTty
       .\tools\scripts\dedupe-solve-rotations.ps1

.EXAMPLE
  .\tools\scripts\run-6x6-enumerate.ps1 -DryRun

.EXAMPLE
  .\tools\scripts\run-6x6-enumerate.ps1 -ResumeFrom 6x6-0C-AAH -NoTty
#>
param(
  [string]$RepoRoot = "",
...
function Invoke-DockerHygiene([string]$Reason) {
  ...
  try {
    & docker compose run --rm web node tools/scripts/docker-hygiene.js 2>&1 | ForEach-Object { Write-Host $_ }
  }
...
if (-not (Test-Path $idPath)) {
  throw "Missing id list: $idPath`nRun: docker compose run --rm web node tools/scripts/build-6x6-unsolved-batch.js --include-jun13-batch"
}
...
Write-Host "When done: .\tools\scripts\run-6x6-enumerate.ps1 -SyncCatalogOnly -NoTty" -ForegroundColor DarkGray
