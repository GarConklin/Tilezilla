 ...
 *
 * Usage (repo root):
 *   node tools/scripts/refresh-solve-queue.js
 *   node tools/scripts/refresh-solve-queue.js --hours 48
 *   node tools/scripts/refresh-solve-queue.js --all-buckets
 *   node tools/scripts/refresh-solve-queue.js --manual 6x6-0C-AZZ,5x5-0A-AAA
 *
 * Docker (compose file at repo root; `.` mounted at `/app` in service `web`):
 *   docker compose run --rm web node tools/scripts/refresh-solve-queue.js
 */
...
function main() {
  ...
  const doc = {
    ...
    note:
      'Run one pending job at a time (docker: pending[].runDocker; host: pending[].run). Regenerate: docker compose run --rm web node tools/scripts/refresh-solve-queue.js',
