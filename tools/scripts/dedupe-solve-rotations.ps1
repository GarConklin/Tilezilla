...
.EXAMPLE
  .\tools\scripts\dedupe-solve-rotations.ps1 -AuditOnly

.EXAMPLE
  .\tools\scripts\dedupe-solve-rotations.ps1 -OpenTail
#>
param(
  [string]$RepoRoot = "",
  [switch]$AuditOnly,
...
try {
  ...
  $out = & python tools/scripts/sync-daily-csv-from-catalog.py 2>&1
