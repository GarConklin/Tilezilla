...
.EXAMPLE
  .\tools\scripts\cleanup-solver-runs-6x6.ps1 -DryRun

.EXAMPLE
  .\tools\scripts\cleanup-solver-runs-6x6.ps1 -Include5x6Streams
#>
param(
  [string]$RepoRoot = "",
  [switch]$DryRun,
...
if (-not $DryRun) {
  ...
  Write-Host "  .\tools\scripts\archive-run-artifacts.ps1" -ForegroundColor DarkGray
}
