import {
  ...
  loadTilebagLayout,
  mergeTilebagLayout,
  stashTilebagLayoutDraft,
} from '/js/tilebag-layout.js';
...
let workingLayout = mergeTilebagLayout(null);
...
function patchBox(key, patch, { live = false } = {}) {
  ...
  if (live) {
    applyTilebagLayout(workingLayout, document.documentElement);
    buildMockPalette();
    els.readout.innerHTML = formatReadout(currentItem);
    renderOverlays();
    requestAnimationFrame(() => renderOverlays());
    stashTilebagLayoutDraft(workingLayout);
    scheduleSave();
    return;
  }
  refresh();
...
function overlayRectForItem(key) {
  ...
  if (isBoxItem(key)) {
    ...
    const top = previewVariant === 'expanded'
      ...
      : tabH + (b.nudgeY || 0);
    return {
      left: `${(b.left / TILEBAG_ART.w) * fw + b.nudgeX}px`,
      top: `${top}px`,
      width: `${(b.width / TILEBAG_ART.w) * fw}px`,
      height: `${trackH}px`,
    };
  }
  ...
  if (key === 'handle') {
    ...
    const left = (h.countX / TILEBAG_ART.w) * fw + (h.handleXOffset || 0) + (h.handleNudgeX || 0);
  ...
  if (key === 'title' || key === 'count') {
    const h = workingLayout.handle;
    const left = (h.countX / TILEBAG_ART.w) * fw + (h.countNudgeX || 0)
      + (key === 'title' ? (h.titleOffsetX || 0) + (h.titleNudgeX || 0) : 0);
    const width = key === 'count'
      ? (h.countW / TILEBAG_ART.w) * fw
      : Math.max(60, (h.countW / TILEBAG_ART.w) * fw);
    return {
      left: `${left}px`,
...
function onOverlayPointerMove(e) {
  ...
  if (isBoxItem(key)) {
    if (previewVariant === 'expanded' && key === 'track' && e.shiftKey) {
      ...
      patchBox(key, {
        left: Math.round(snapshot.left + (dx / frameW) * TILEBAG_ART.w),
...
function formatReadout(key) {
  ...
  if (key === 'tiles') {
    ...
    return `<strong>${def.label}</strong><br />cell ${t.cell}px · gap ${t.gap}px · vNudge ${t.vNudge}px · hScale ${t.thumbHScale}<br />fine-tune field <strong>${field}</strong> = ${t[field]}${mode}`;
  }
  if (key === 'glow') {
...
function updateKeysHelp() {
  ...
  if (currentItem === 'tiles') {
    ul.innerHTML = [
      '<li>Scroll — tile size (cell) · Shift+scroll — move up/down (vNudge)</li>',
      '<li>Ctrl+scroll — gap between tiles</li>',
      '<li>Drag yellow tiles box — up/down · Shift+drag — resize</li>',
      '<li>[ ] — cycle hScale / fitInsetY · ↑↓ — vNudge · ←→ — cell</li>',
    ].join('');
    return;
  }
  if (currentItem === 'glow') {
  ...
  if (['expandedLayout', 'container'].includes(currentItem)) {
    if (SECTION_FIELDS[currentItem]) {
      ul.innerHTML = [
        ...
        '<li>[ ] — previous / next field in section</li>',
      ].join('');
      return;
    }
...
function refresh() {
  ...
  els.reportOut.value = buildTilebagLayoutReport(workingLayout);
  stashTilebagLayoutDraft(workingLayout);
  refreshFieldGrid();
  requestAnimationFrame(() => renderOverlays());
  updateKeysHelp();
...
function onKeyDown(e) {
  if (tuningBlocked(e.target)) return;
  if (SECTION_FIELDS[currentItem] && (e.key === '[' || e.key === ']')) {
...
async function saveToFile({ quiet = false } = {}) {
  ...
  saveInFlight = true;
  try {
    const res = await fetch('/api/dev/save-tilebag-layout', {
      method: 'POST',
    ...
    clearTilebagLayoutCache();
    clearTilebagLayoutDraft();
    localStorage.setItem('tilezilla:tilebag-layout-version', String(Date.now()));
    window.dispatchEvent(new CustomEvent('tilezilla:tilebag-layout-saved'));
    els.status.textContent = quiet ? 'Auto-saved' : 'Saved to data/tilebag_layout.json';
  } catch (err) {
...
async function init() {
  ...
  document.getElementById('reloadBtn')?.addEventListener('click', async () => {
    clearTilebagLayoutCache();
    clearTilebagLayoutDraft();
    workingLayout = cloneLayout(await loadTilebagLayout({ force: true }));
    refresh();
  ...
  try {
    workingLayout = cloneLayout(await loadTilebagLayout());
  } catch {
    workingLayout = mergeTilebagLayout(null);
  }
