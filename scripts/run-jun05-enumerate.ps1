<#
.SYNOPSIS
  Full enumeration (Docker) for 44 new levels from Jun 5 ingest.

.DESCRIPTION
  From data/tilepz solves 05 June  2026.txt (merge remaps pasted ids):

    5x6-0B-AXV … 5x6-0B-AZC (34) — pasted AMJ…ANQ reused existing codes (bag mismatch)
    6x6-0B-ACP … 6x6-0B-ACY (10) — pasted 6x6-0C-AAO…AAX → 0B tier

.EXAMPLE
  .\scripts\run-jun05-enumerate.ps1 -Phase 5x6 -NoTty

.EXAMPLE
  .\scripts\run-jun05-enumerate.ps1 -Phase 6x6 -SyncCatalog -NoTty

.EXAMPLE
  .\scripts\run-jun05-enumerate.ps1 -Phase All -SyncCatalog -ContinueOnError -NoTty

.EXAMPLE
  .\scripts\run-jun05-enumerate.ps1 -DryRun -Phase 5x6
...
function Get-Jun05Ids5x6 {
  $start = ConvertTo-ThreeLetterIndex 'AXV'
  $end = ConvertTo-ThreeLetterIndex 'AZC'
...
function Get-Jun05LevelIdsBuiltin([string]$ListPhase) {
  $start6 = ConvertTo-ThreeLetterIndex 'ACP'
  $end6 = ConvertTo-ThreeLetterIndex 'ACY'
  $six = [System.Collections.Generic.List[string]]::new()
  for ($i = $start6; $i -le $end6; $i++) {
    $six.Add("6x6-0B-$(ConvertFrom-ThreeLetterIndex $i)")
  }
  $five = Get-Jun05Ids5x6
  switch ($ListPhase) {
    '5x6' { return @($five) }
    '6x6' { return @($six) }
    'All' { return @($five + $six) }
...
function Get-LevelIds([string]$ListPhase) {
  if ($SyncCatalogOnly) {
    Write-Host "Using built-in Jun 5 id list (no Docker list step)." -ForegroundColor DarkGray
    return Get-Jun05LevelIdsBuiltin -ListPhase $ListPhase
  }
  ...
  try {
    ...
    if ($LASTEXITCODE -ne 0 -or $ids.Count -eq 0) {
      if ($lines -match 'marked for removal|cannot be started') {
        ...
        Invoke-DockerHygiene "before list-jun05 retry"
        $lines = @(& docker @dockerArgs 2>&1 | ForEach-Object { "$_" })
        $ids = @($lines | Where-Object { $_ -match '^\d+x\d+-\S+-\S+$' })
      }
    }
    if ($ids.Count -eq 0) {
      Write-Host "Falling back to built-in Jun 5 id list." -ForegroundColor Yellow
      return Get-Jun05LevelIdsBuiltin -ListPhase $ListPhase
    }
    return $ids
  }
...
Invoke-DockerHygiene "before jun05 run"

if ($SyncCatalogOnly) {
  $SyncCatalog = $true
}

Write-Step "Jun 5 enumerate phase=$Phase (MaxSol=$MaxSol)"
...
$logPath = Join-Path $logRoot "jun05-enumerate-$Phase-$stamp.log"
$progressPath = Join-Path $logRoot "jun05-enumerate-$Phase-$stamp.ndjson"
...
$log.WriteLine("# jun05-enumerate phase=$Phase stamp=$stamp maxSol=$MaxSol")
...
if (-not $DryRun -and -not $SyncCatalog) {
  ...
  Write-Host "  .\scripts\run-jun05-enumerate.ps1 -Phase $Phase -SyncCatalog -NoTty" -ForegroundColor DarkGray
}
