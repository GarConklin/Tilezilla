<#
.SYNOPSIS
  Full enumeration (Docker) for 4 new levels from May 30 ingest.

.DESCRIPTION
  Runs solves/solve-level.js for levels added from
  data/tilepz solves 30 may 2026a.txt:

    6x6-0B-ABY … 6x6-0B-ACB

  Remote labels 6x6-0A-ABE…ABH were wrong tier; ingest remapped to 0B.

.EXAMPLE
  .\scripts\run-may30-enumerate.ps1

.EXAMPLE
  .\scripts\run-may30-enumerate.ps1 -SyncCatalog

.EXAMPLE
  .\scripts\run-may30-enumerate.ps1 -DryRun

.EXAMPLE
  .\scripts\run-may30-enumerate.ps1 -ResumeFrom "6x6-0B-ABV" -ContinueOnError
...
function Get-LevelIds {
  ...
  try {
    $dockerArgs = @(
      'compose', 'run', '--rm', 'web',
      'node', 'scripts/list-may30-new-level-ids.js'
    )
    ...
    if ($LASTEXITCODE -ne 0) {
      throw "list-may30-new-level-ids.js failed: $($lines -join "`n")"
    }
...
Write-Step "May 30 enumerate (MaxSol=$MaxSol)"
$ids = Get-LevelIds
...
$logPath = Join-Path $logRoot "may30-enumerate-6x6-$stamp.log"
$progressPath = Join-Path $logRoot "may30-enumerate-6x6-$stamp.ndjson"
...
$log.WriteLine("# may30-enumerate stamp=$stamp maxSol=$MaxSol")
...
if (-not $DryRun -and -not $SyncCatalog) {
  ...
  Write-Host "  .\scripts\run-may30-enumerate.ps1 -SyncCatalog" -ForegroundColor DarkGray
}
