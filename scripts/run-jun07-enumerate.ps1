<#
.SYNOPSIS
  Full enumeration (Docker) for 37 new levels from Jun 7 ingest.

.DESCRIPTION
  From data/tilepz solves 07 jun 2026.txt (merge remaps pasted ids):

    5x6-0B-AZD … 5x6-0B-BAH (31) — pasted ANR…AOW (AOS dup of AEA, skipped)
    6x6-0B-ACZ … 6x6-0B-ADE (6) — pasted 6x6-0C-AAY…AAC + 6x6-0A-AAT → 0B tier

.EXAMPLE
  .\scripts\run-jun07-enumerate.ps1 -Phase 5x6 -NoTty

.EXAMPLE
  .\scripts\run-jun07-enumerate.ps1 -Phase 6x6 -SyncCatalog -NoTty

.EXAMPLE
  .\scripts\run-jun07-enumerate.ps1 -Phase All -SyncCatalog -ContinueOnError -NoTty

.EXAMPLE
  .\scripts\run-jun07-enumerate.ps1 -DryRun -Phase 5x6
...
function Get-jun07Ids5x6 {
  $start = ConvertTo-ThreeLetterIndex 'AZD'
  $end = ConvertTo-ThreeLetterIndex 'BAH'
...
function Get-jun07LevelIdsBuiltin([string]$ListPhase) {
  $start6 = ConvertTo-ThreeLetterIndex 'ACZ'
  $end6 = ConvertTo-ThreeLetterIndex 'ADE'
  $six = [System.Collections.Generic.List[string]]::new()
  for ($i = $start6; $i -le $end6; $i++) {
    $six.Add("6x6-0B-$(ConvertFrom-ThreeLetterIndex $i)")
  }
  $five = Get-jun07Ids5x6
  switch ($ListPhase) {
    '5x6' { return @($five) }
    '6x6' { return @($six) }
    'All' { return @($five + $six) }
...
function Get-LevelIds([string]$ListPhase) {
  if ($SyncCatalogOnly) {
    Write-Host "Using built-in Jun 7 id list (no Docker list step)." -ForegroundColor DarkGray
    return Get-jun07LevelIdsBuiltin -ListPhase $ListPhase
  }
  ...
  try {
    ...
    if ($LASTEXITCODE -ne 0 -or $ids.Count -eq 0) {
      if ($lines -match 'marked for removal|cannot be started') {
        ...
        Invoke-DockerHygiene "before list-jun07 retry"
        $lines = @(& docker @dockerArgs 2>&1 | ForEach-Object { "$_" })
        $ids = @($lines | Where-Object { $_ -match '^\d+x\d+-\S+-\S+$' })
      }
    }
    if ($ids.Count -eq 0) {
      Write-Host "Falling back to built-in Jun 7 id list." -ForegroundColor Yellow
      return Get-jun07LevelIdsBuiltin -ListPhase $ListPhase
    }
    return $ids
  }
...
Invoke-DockerHygiene "before jun07 run"

if ($SyncCatalogOnly) {
  $SyncCatalog = $true
}

Write-Step "Jun 7 enumerate phase=$Phase (MaxSol=$MaxSol)"
...
$logPath = Join-Path $logRoot "jun07-enumerate-$Phase-$stamp.log"
$progressPath = Join-Path $logRoot "jun07-enumerate-$Phase-$stamp.ndjson"
...
$log.WriteLine("# jun07-enumerate phase=$Phase stamp=$stamp maxSol=$MaxSol")
...
if (-not $DryRun -and -not $SyncCatalog) {
  ...
  Write-Host "  .\scripts\run-jun07-enumerate.ps1 -Phase $Phase -SyncCatalog -NoTty" -ForegroundColor DarkGray
}
