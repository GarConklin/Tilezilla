...
/**
 * Build jun22-enumerate-manifest.json from a Jun 22 ingest log (WRITE lines).
 * Splits ingested level IDs into 4 parallel enumeration batches.
 *
 * Usage:
 *   node scripts/gen-jun22-enumerate-manifest.js
 *   node scripts/gen-jun22-enumerate-manifest.js data/solver-runs/ingest-batch-YYYYMMDD-HHMMSS.log
 */
...
const OUT = path.join(RUNS, 'jun22-enumerate-manifest.json');
const BATCH_CHUNKS = 4;
const BATCH_FILE = 'data/5x6 tilepz solves 22 June  2026.txt';

function findLatestIngestLog() {
  const logs = findJun22IngestLogs();
...
function findJun22IngestLogs(explicitPath) {
  if (explicitPath) {
  ...
  return fs
    ...
    .filter((p) => {
      ...
      return text.includes('WRITE /app/solves/5x6-0B-BSV.json') || text.includes('WRITE /app/solves/5x6-0B-CCC.json');
    })
...
function main() {
  ...
  const logPaths = explicit
    ? findJun22IngestLogs(explicit)
    : findJun22IngestLogs();

  if (!logPaths.length) {
    console.error('No Jun 22 ingest log found. Run ingest first, or pass log path.');
    process.exit(1);
  }
