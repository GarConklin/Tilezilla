<#
.SYNOPSIS
  Full enumeration (Docker) for 5x6 levels from Jun 9 ingest (BAI…BEZ).

.DESCRIPTION
  From data/tilepz solves 09 jun 2026.txt (merge remaps pasted ids to 5x6-0B-BAI…BEZ):

    Batch1  5x6-0B-BAI … 5x6-0B-BBZ
    Batch2  5x6-0B-BCA … 5x6-0B-BDQ
    Batch3  5x6-0B-BDR … 5x6-0B-BEZ

  Run three batches in parallel (separate terminals):

    .\scripts\run-jun09-enumerate.ps1 -Phase Batch1 -NoTty -ContinueOnError
    .\scripts\run-jun09-enumerate.ps1 -Phase Batch2 -NoTty -ContinueOnError
    .\scripts\run-jun09-enumerate.ps1 -Phase Batch3 -NoTty -ContinueOnError

.EXAMPLE
  .\scripts\run-jun09-enumerate.ps1 -Phase Batch1 -NoTty

.EXAMPLE
  .\scripts\run-jun09-enumerate.ps1 -Phase All -SyncCatalog -ContinueOnError -NoTty

.EXAMPLE
  .\scripts\run-jun09-enumerate.ps1 -DryRun -Phase Batch2

  Stop: .\scripts\stop-solve-docker-runs.ps1 (2nd terminal), then Ctrl+C here.
  Status: .\scripts\show-solve-runs-status.ps1 -Watch
#>
param(
  [ValidateSet('Batch1', 'Batch2', 'Batch3', 'All')]
  [string]$Phase = 'All',
...
function Get-IdsForRange([string]$fromCode, [string]$toCode) {
  $start = ConvertTo-ThreeLetterIndex $fromCode
  $end = ConvertTo-ThreeLetterIndex $toCode
...
function Get-jun09LevelIdsBuiltin([string]$ListPhase) {
  $batch1 = Get-IdsForRange 'BAI' 'BBZ'
  $batch2 = Get-IdsForRange 'BCA' 'BDQ'
  $batch3 = Get-IdsForRange 'BDR' 'BEZ'
  switch ($ListPhase) {
    'Batch1' { return @($batch1) }
    'Batch2' { return @($batch2) }
    'Batch3' { return @($batch3) }
    'All' { return @($batch1 + $batch2 + $batch3) }
    default { return @() }
  }
}

Invoke-DockerHygiene "before jun09 run"

if ($SyncCatalogOnly) {
  $SyncCatalog = $true
}

Write-Step "Jun 9 enumerate phase=$Phase (MaxSol=$MaxSol)"
$ids = Get-jun09LevelIdsBuiltin -ListPhase $Phase
...
$logPath = Join-Path $logRoot "jun09-enumerate-$Phase-$stamp.log"
$progressPath = Join-Path $logRoot "jun09-enumerate-$Phase-$stamp.ndjson"
...
$log.WriteLine("# jun09-enumerate phase=$Phase stamp=$stamp maxSol=$MaxSol")
...
if (-not $DryRun -and -not $SyncCatalog) {
  ...
  Write-Host "  .\scripts\run-jun09-enumerate.ps1 -Phase All -SyncCatalogOnly -NoTty" -ForegroundColor DarkGray
}
