<#
.SYNOPSIS
  Full enumeration (Docker) for 4 new levels from May 28 ingest.

.DESCRIPTION
  Runs solves/solve-level.js for levels added from
  data/tilepz solves 28 may 2026a.txt:

    6x6-0B-ABU … 6x6-0B-ABX

  Remote labels 6x6-0A-ABA…ABD were wrong tier; ingest remapped to 0B.

.EXAMPLE
  .\scripts\run-may28-enumerate.ps1

.EXAMPLE
  .\scripts\run-may28-enumerate.ps1 -SyncCatalog

.EXAMPLE
  .\scripts\run-may28-enumerate.ps1 -DryRun

.EXAMPLE
  .\scripts\run-may28-enumerate.ps1 -ResumeFrom "6x6-0B-ABV" -ContinueOnError
...
function Get-LevelIds {
  ...
  try {
    ...
    if ($LASTEXITCODE -ne 0) {
      throw "list-may28-new-level-ids.js failed: $($lines -join "`n")"
    }
...
Write-Step "May 28 enumerate (MaxSol=$MaxSol)"
$ids = Get-LevelIds
...
$logPath = Join-Path $logRoot "may28-enumerate-6x6-$stamp.log"
$progressPath = Join-Path $logRoot "may28-enumerate-6x6-$stamp.ndjson"
...
$log.WriteLine("# may28-enumerate stamp=$stamp maxSol=$MaxSol")
...
if (-not $DryRun -and -not $SyncCatalog) {
  ...
  Write-Host "  .\scripts\run-may28-enumerate.ps1 -SyncCatalog" -ForegroundColor DarkGray
}
