<#
.SYNOPSIS
  Full enumeration (Docker) for 5x6 levels from Jun 22 ingest.

.DESCRIPTION
  Ingest: data/5x6 tilepz solves 22 June  2026.txt
  Manifest: data/solver-runs/jun22-enumerate-manifest.json (4 batches)

  Run four batches in parallel (separate terminals):

    .\scripts\run-jun22-enumerate.ps1 -Phase Batch1 -NoTty -ContinueOnError
    .\scripts\run-jun22-enumerate.ps1 -Phase Batch2 -NoTty -ContinueOnError
    .\scripts\run-jun22-enumerate.ps1 -Phase Batch3 -NoTty -ContinueOnError
    .\scripts\run-jun22-enumerate.ps1 -Phase Batch4 -NoTty -ContinueOnError

  After all four finish:

    .\scripts\run-jun22-enumerate.ps1 -Phase All -SyncCatalogOnly -NoTty

  Regenerate manifest after ingest:

    node scripts/gen-jun22-enumerate-manifest.js data/solver-runs/ingest-batch-*.log
...
function Get-jun22Manifest {
  $manifestPath = Join-Path $RepoRoot "data/solver-runs/jun22-enumerate-manifest.json"
  if (-not (Test-Path -LiteralPath $manifestPath)) {
    Write-Host "Generating manifest..." -ForegroundColor DarkYellow
    & node (Join-Path $RepoRoot "scripts/gen-jun22-enumerate-manifest.js")
  }
  if (-not (Test-Path -LiteralPath $manifestPath)) {
    throw "Missing manifest: $manifestPath — run ingest then: node scripts/gen-jun22-enumerate-manifest.js"
  }
  return Get-Content -Raw -LiteralPath $manifestPath | ConvertFrom-Json
}

function Get-jun22LevelIds([string]$ListPhase) {
  $manifest = Get-jun22Manifest
  if ($ListPhase -eq 'All') {
...
Invoke-DockerHygiene "before jun22 run"

if ($SyncCatalogOnly) {
  $SyncCatalog = $true
}

Write-Step "Jun 22 enumerate phase=$Phase (MaxSol=$MaxSol)"
$ids = @(Get-jun22LevelIds -ListPhase $Phase)

if ($ids.Count -eq 0) {
  throw "No level IDs loaded for phase '$Phase'. Run ingest + node scripts/gen-jun22-enumerate-manifest.js"
}
...
$logPath = Join-Path $logRoot "jun22-enumerate-$Phase-$stamp.log"
$progressPath = Join-Path $logRoot "jun22-enumerate-$Phase-$stamp.ndjson"
...
$log.WriteLine("# jun22-enumerate phase=$Phase stamp=$stamp maxSol=$MaxSol")
...
if (-not $DryRun -and -not $SyncCatalog) {
  ...
  Write-Host "  .\scripts\run-jun22-enumerate.ps1 -Phase All -SyncCatalogOnly -NoTty" -ForegroundColor DarkGray
}
