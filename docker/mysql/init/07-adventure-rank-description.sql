-- Idempotent upgrade: rank_description on adventure_rank (canonical: data/adventure_ranks.json)

USE tilegame;

DROP PROCEDURE IF EXISTS tilegame_add_adventure_rank_description;

DELIMITER //
CREATE PROCEDURE tilegame_add_adventure_rank_description()
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM information_schema.COLUMNS
        WHERE TABLE_SCHEMA = DATABASE()
          AND TABLE_NAME = 'adventure_rank'
          AND COLUMN_NAME = 'rank_description'
    ) THEN
        ALTER TABLE adventure_rank
            ADD COLUMN rank_description VARCHAR(255) NULL AFTER rank_name;
    END IF;
END//
DELIMITER ;

CALL tilegame_add_adventure_rank_description();
DROP PROCEDURE tilegame_add_adventure_rank_description;
